        ...
        <div class="disribution-list">
            ...
            <div class="search-wrap">
                <el-form :inline="true" class="demo-form-inline">
                    <el-form-item :label="$t('generateTime')">
            ...
            <div class="table-wrap" ref="myTable" v-loading="loading" :element-loading-text="$t('loadingText')">
                <el-table
                    ...
                    <el-table-column
                        ...
                        :label="$t('operation')">
                        <template slot-scope="scope">
                            <el-button type="text" @click="fetchDetail(scope.row)">{{ $t('detail') }}</el-button>
                            <el-button type="text" @click="downloadPdf(scope.row)">{{ $t('downloadBarCode') }}</el-button>
                            <el-button type="text"
...
import { baseUrl, device } from '@/common/api.js'
...
export default {
    ...
    methods: {
        downloadPdf (item) {
            // this.$axios.get(device.downloadbatchpdf, {
            //     params: {
            //         uid: item.uid
            //     }
            // }).then(res => {})
            window.open(location.origin + baseUrl + device.downloadbatchpdf + '?uid=' + item.uid, '_blank')
        },
        chooseDevice (val) {
        ...
        fetchData (pageNum, pageSize) {
            ...
            this.loading = true
            this.$axios.post(device.listcomsumablesbatch, {
        ...
        fetchDetail (item) {
            this.$emit('getBatchNo', item.batchNo)
        },
        updateDetail () {
        ...
        // 设置序列号状态
        setstate (row) {
            this.$axios.post(device.setcomsumablesbatchstate, {
                batchUid: row.uid,
